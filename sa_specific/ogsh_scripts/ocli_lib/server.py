...
class Server:
	...
	def __init__(self, server_id=None, server_name=None, create=0, lifecycle=None):
		# if they gave us the name, we'll fetch the id and vice versa
		if server_id:
			server_id = string.strip(server_id)
			self.__findServerByID(server_id)
		elif server_name:
			self.name = server_name
		else:
			raise

		self.__setupServerInfo()


	def __init__(self, mac, custattrdict, lifecycle):
		print "in the other constructor"
		#stuff

	# this method is called by the constructors to set up some basic
	# info for this object
	def __setupServerInfo(self):
