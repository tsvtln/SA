...
# setup java binary to be used by table_dict.py below.
JAVABIN="`echo /opt/opsware/*/bin/java | head -1`"
export JAVABIN
...
# If we have not yet built the table id fetcher on this box.
if { [ \! -f "TableIdFetcher.class" ]; } then {
  # build it.
  ./build_java_stuff
} fi
