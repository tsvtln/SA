#!/bin/sh
########################################################
# Script Name - FindSpaceLessThanX.sh
# Function - Given a value - find all filesystems less 
# than that value in free space. 
# Argument - Value as in integer - interpreted as %
########################################################
cwd=`pwd`
cd "`dirname $0`"
MYDIR=`pwd`
...
ARG_LIST=$1
SCRIPT_NAME=SpaceUsedMoreThanX
...
if  [ "x$ARG_LIST" == "x" ];
then
        echo "Missing argument: Please pass the Space Threshold"
        exit 1
fi
...
then
   echo "Argument value  should be between 0 and 100 "
   exit 1
fi
...
$TTLG_CMD -n$x -l$TTLG_USER -s $LINUX_SCRIPT_LOCATION/$LINUX_SCRIPT_NAME $ARG_LIST
        ...
        then
                $TTLG_CMD -n$x -l$WINDOWS_TTLG_USER -s $WINDOWS_SCRIPT_LOCATION/$WINDOWS_SCRIPT_NAME $ARG_LIST
        fi
else
