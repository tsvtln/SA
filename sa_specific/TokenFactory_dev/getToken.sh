...
if ( [ -f "/opt/opsware/twist/extlib/client/twistclient.jar" ]; ) then {
  sJavaCmd="/opt/opsware/jdk1.6/bin/java -classpath $0:/opt/opsware/twist/extlib/client/twistclient.jar:/opt/opsware/twist/lib/twist.jar:/opt/opsware/twist/lib/common-latest.jar"
} elif ( [ -f "/opt/opsware/twist/lib/client/twistclient.jar" ]; ) then {
  ...
  sJavaCmd="`ls /cust/j*/bin/java | head -1` -classpath $0:/cust/twist/lib/client/twistclient.jar:/cust/twist/lib/twist.jar:/cust/twist/lib/common-latest.jar:/cust/twist/lib/commons-codec-1.3.jar"
} else {
...
nUserID=`$sPythonCmd -c "from coglib import spinwrapper;import string;import sys;sys.stdout.write(string.replace(str(spinwrapper.SpinWrapper()._AAAAaaUser.getList(restrict={'username':'$sUserName','user_status':'ACTIVE'})[0][0]),'L',''))"`
