#!/bin/sh
#######################################################
#Script - FindProcessMoreThanXCpu.sh
#Function - Finds processes that are more than X%
#Argument - CPU threshold as an integer
#######################################################
...
ARG_LIST=$1
SCRIPT_NAME=ListProcsGreaterthanXCpu
...
WINDOWS_RESULTS_FILE=/tmp/$SCRIPT_NAME.$$
...
if  [ "x$ARG_LIST" == "x" ];
then
        echo "Missing argument: Please pass the CPU Threshold"
        exit 1
fi
...
then
   echo "CPU value should be between 0 and 100 "
   exit 1
fi
...
$TTLG_CMD -n$x -l$TTLG_USER -s $LINUX_SCRIPT_LOCATION/$LINUX_SCRIPT_NAME $ARG_LIST
        ...
        then
                $TTLG_CMD -n$x -l$WINDOWS_TTLG_USER -s $WINDOWS_SCRIPT_LOCATION/$WINDOWS_SCRIPT_NAME /P $ARG_LIST  | grep -v System | sort  -r -k 3  > $WINDOWS_RESULTS_FILE
                startcount=1
