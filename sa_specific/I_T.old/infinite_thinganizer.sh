...
PYTHONPATH=/lc/blackshadow:/cust/usr/blackshadow:/cust/usr/blackshadow/spin

/lc/bin/python curie_classes.py | sort > classes.txt
if [ $? != 0 ]; then
  echo 'run of curie_classes failed' && exit 1
fi
for line in `cat classes.txt`; do
  echo `date '+%Y-%m-%d_%H:%M:%S'` AAA $line
  objcount=`/lc/bin/python obj_count.py $line`
  if [ $? != 0 ]; then
    echo 'run of obj_count' $line 'failed' && exit 1
  fi
  if test $objcount -lt 100000 ; then
    /lc/bin/python table_checker.py -c $line --simple >> table_checker.out
    if [ $? != 0 ]; then
      echo 'run of table_checker' $line 'failed'
    fi
  else
    /lc/bin/python table_dict.py truths.txt $line >> table_dict.out
    if [ $? != 0 ]; then
...
while [ $retcode = 0 ] ; do
  ...
  /lc/bin/python mk_rechk.py obj.txt recheck.sh
  if [ $? != 0 ]; then
  ...
  if [ $retcode = 0 ] ; then
    /lc/bin/python -u sync_objects.py obj.txt
  fi
done
