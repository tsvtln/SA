...
sShadowBotName="agent"

# Extract the basename of this script.
sName="`basename $0`"
sSBOpletFileName="${sName}_oplet.py"
sSBOpletFilePath="/opt/opsware/agent/pylibs/cogbot/base/${sSBOpletFileName}"
...
self.wfile.write("$sShadowBotName internal system environment:\n")

self.wfile.write(pprint.pformat(os.environ.data))
...
(url_obj,headers)=urlopen.httpReply(urlopen.httpUrlRequest("https://localhost:1002/${sSBOpletFileName}",ctx=certmaster.getContextByName("spin","spin.srv","opsware-ca.crt"),connect_timeout=
