...
# [X] modified to allow it to work on native Windows python build.
#
# arch todo:
...
def link(src_node, dst_node):
  dbg(1, "link(%s, %s)", (repr(src_node), repr(dst_node)))
...
def spin_call_raw(spin, xmlrpc_mthd, args=(), kwargs={}):
  dbg(2, "SC: %s(args=%s, kwargs=%s)", (xmlrpc_mthd, args, kwargs))
...
def spin_call(spin, xmlrpc_mthd, args=(), kwargs={}, ret_ex=1):
  try:
    ...
    if ( ret_ex ):
      ...
      dbg(1, string.join( apply( traceback.format_exception, sys.exc_info() ), ""))
      return None
...
def sanitize_spin_result(obj):
  ...
  elif ( type(obj) not in (types.IntType, types.LongType, types.FloatType, types.NoneType) ):
    return str(obj)
  else:
    return obj
...
"""
  def __init__(self, outfilename):
    self.outfile = gzip.open(outfilename,'wb')
    self.count = 0
    self.index = {}
    self.finalized = 0
...
class ObjReader:
  ...
  def _init_infile(self, infilename):
    ...
    self.infile = open(fo_name,'w+b')
    try:
      os.unlink(fo_name)
    except:
      warn("%s: Failed to unlink. [%s]", (fo_name, sys.exc_info()[1]))
    BS = 4096
    buf = fi.read(BS)
    while (len(buf) > 0):
  ...
  def get(self, oid, def_ret=None):
    soid = str(oid)
    if ( self.index.has_key(soid) ):
      idx = self.index[soid]
      dbg(2, "ObjReader.get: soid: %s, idx: %d", (soid, idx))
      self.infile.seek(idx)
  ...
  for obj in objs:
    ...
    if ( type(params) == types.DictType ):
      ...
      if ( type(dvc_ids) in (types.ListType, types.TupleType) ):
        ...
        oid_stack.append(Oid("Device", dvc_ids))

    # Assign the results of this obj.
    results = all_results.get(sid, None)
    if ( results is not None ):
      ex_obj['getResults'] = results
      if ( type(results)==types.DictType ):
        for key in ("remediate_id",):
          ssid = results.get(key, None)
          if ( ssid != None ):
            dbg(1, "Found subsession %s via <%s> result value on session %s.", (ssid, key, sid))
            oid_stack.append(Oid("Session", ssid))
        ss = results.get("sessions", None)
        if ( ss != None ):
          try:
            ssoids = map(lambda s:Oid("Session", s[0]), ss)
            dbg(1, "Found subsessions %s via <sessions> result value on session %s.", (repr(map(lambda soid:soid.id,ssoids)), sid))
            oid_stack.extend(ssoids)
          except:
            warn("Unable to decode <sessions> result value on session %s.\n%s", (sid, repr(ss)))
            dbg(1, last_exc())
    ...
    if ( md is not None ):
      ...
      if ( type(md) == types.DictType ):
        if ( md.get('NEXT_SESSION_ID',None) != None ):
          ssid = str(md['NEXT_SESSION_ID'])
          dbg(1, "Found subsession %s via <NEXT_SESSION_ID> metadata on session %s.", (ssid, sid))
          oid_stack.append(Oid("Session", ssid))
        if ( md.get('JOB_ID', None) != None ):
          ssid = str(md['JOB_ID'])
          dbg(1, "Found subsession %s via <JOB_ID> metadata on session %s.", (ssid, sid))
          oid_stack.append(Oid("Session", ssid))
...
def ex_SessionCommand(spin, objs, oid_stack):
  ...
  for obj in objs:
    ...
    if ( results is not None ):
      ex_obj['getResults'] = results
      if ( type(results)==types.DictType ):
        for key in ("$session_id", "remediate_id"):
          sid = results.get(key, None)
          if ( sid != None ):
            dbg(1, "Found subsession %s via <%s> result value on command %s.", (sid, key, cid))
            oid_stack.append(Oid("Session", sid))
        ss = results.get("sessions", None)
        if ( ss != None ):
          try:
            soids = map(lambda s:Oid("Session", s[0]), ss)
            dbg(1, "Found subsessions %s via <sessions> result value on command %s.", (repr(map(lambda soid:soid.id,soids)), cid))
            oid_stack.extend(soids)
          except:
            warn("Unable to decode <sessions> result value on command %s.\n%s", (cid, repr(ss)))
            dbg(1, last_exc())
    ...
    if ( obj['way_script_version_id'] != None ):
      oid_stack.append(Oid("WayScriptVersion", obj['way_script_version_id']))
...
# Objects are initially dumped generically, then we invoke an extension
...
"""
  # Perform a sanity check to make sure the object exists in the DB.
  try:
    if ( len(getattr(spin, oid.cls).getIDList(restrict={g_cls_id_flds[oid.cls]:oid.id})) == 0 ): 
      raise ValueError(str(oid))
  except:
    error("%s %s not found in database.", (oid.cls,str_id(oid.id)))
    return

  # Create an object writter.
  of_name = "%s%s.pkl.gz" % (oid.cls, oid.id)
  info("Dumping %s to %s", (oid.cls, repr(of_name)))
  ...
  while oid_stack.has_more():
    ...
    while (ids):
      ...
      for obj in objs:
        ...
        cf_oid = oid_of_obj(obj)
        found_ids.append(cf_oid.id)

        # Let user know what we are currently emitting.
        info("Emitting %s", (cf_oid,))

        # emit it.
        ow.emit_obj(cf_oid, obj)
...
CSS = """
<!--
html { background: #8BB381; color: black }
body { background: #8BB381; color: black; font-family: monospace; }
th { text-align: left }
...
class P(HtmlElem): pass
class S(HtmlElem): pass
...
class HtmlPage:
  ...
  def write(self, out_path):
    info(out_path)
    open(out_path,'wb').write(str(self))
...
def mk_html_obj_link(oid, body, o_r=None):
  """Creates an html anchor element.  If <o_r> is passed, then it will validate that
the oid actually exists, and if not, emit a strikeout element instead.
"""
  if ( (o_r is not None) and (not o_r.has_oid(oid)) ):
    return S(body)
...
def html_Session_fmt(o_r, sess, top_fields, bot_fields):
  ...
  if ( way_script_version_id is not None ):
    ...
    ws = o_r.get(ws_oid)
    sess['way_script_version_id'] = RawHtml("%s~%s" % (mk_html_obj_link(ws_oid, ws['script_name']),
  ...
  if ( type(acct_ids) == types.ListType ):
    ...
    for acct_id in acct_ids:
      ...
      accts.append(mk_html_obj_link(aoid, o_r.get(aoid,{"acct_name":acct_id})["acct_name"]))
    sess['account_ids'] = RawHtml(accts)
  ...
  if ( type(params) == types.DictType ):
    dvc_ids = params.get('dvc_ids', None)
    if ( type(dvc_ids) in (types.ListType, types.TupleType) ):
      params['dvc_ids'] = RawHtml(map(lambda dvc_id,o_r=o_r:mk_html_obj_link(Oid("Device", dvc_id), dvc_id, o_r), dvc_ids))
    elif ( is_num(dvc_ids) ):
      params['dvc_ids'] = mk_html_obj_link(Oid("Device", dvc_ids), dvc_ids, o_r)

  # getResults field:
  results = sess.get("getResults", None)
  if ( type(results) == types.DictType ):
    for key in ("remediate_id",):
      ssid = results.get(key, None)
      if ( ssid is not None ):
        results[key] = mk_html_obj_link(Oid("Session", ssid), ssid)
    ss = results.get("sessions", None)
    if ( ss is not None ):
      try:
        for s in ss:
          s[0] = mk_html_obj_link(Oid("Session", s[0]), s[0])
        results["sessions"] = RawHtml(ss)
      except:
        sid = sess.get("session_id", "**not found**")
        warn("Unable to encode <sessions> result value on session %s.\n  %s", (sid, ss))
...
def html_Session_full(o_r, oid, out_dir):
  ...
  for cmd_id in cmd_ids:
    ...
    msi_oid = Oid("MegaServiceInstance", cmd['srvc_inst_id'])
    msi = o_r.get(msi_oid)
    if ( msi is not None ):
      cmd['host'] = mk_html_obj_link(msi_oid, msi['system_name'])
      cmd['port'] = msi['port']
    else:
      cmd['host'] = S(str(msi_oid))
...
def html_SessionCommand_fmt(o_r, cmd, top_fields, bot_fields):
  ...
  if ( way_script_version_id is not None ):
    ...
    ws_oid = Oid("WayScript", wsv['way_script_id'])
    ws = o_r.get(ws_oid)
    cmd['way_script_version_id'] = RawHtml("%s~%s" % (mk_html_obj_link(ws_oid, ws['script_name']),
                                                      mk_html_obj_link(wsv_oid, wsv['version'])))
  ...
  if ( msi is not None ):
    ...
    cmd['srvc_inst_id'] = S(str(msi_oid))
  ...
  if ( len(module) == 2 ):
    ...
    cmd['module'] = RawHtml("%s~%s" % (module[0], module[1]))
  ...
  if ( type(results) == types.DictType ):
    for key in ("$session_id", "remediate_id"):
      sid = results.get(key, None)
      if ( sid is not None ):
        results[key] = mk_html_obj_link(Oid("Session", sid), sid)
    ss = results.get("sessions", None)
    if ( ss is not None ):
      try:
        for s in ss:
        ...
        cid = cmd.get("session_cmd_id", "**not found**")
        warn("Unable to encode <sessions> result value on command %s.\n  %s", (cid, ss))
...
def html_WayScriptVersion_full(o_r, oid, out_dir):
  ...
  open(raw_src_out_path, 'wb').write(src)
...
"""
  ...
  g_html_page_header = HtmlElemList(
    Table(Tr(
       ...
       Td(H1("The Way", Br(), Font("(it should have been)", size=-1)),
          ...
          "Spin Version %s" % hdr.get('spin_ver','(**no spin version listed**)'), Br(), align="center"),
  ...
  open(os.path.join(out_dir, "101.gif"), 'wb').write(zlib.decompress(base64.decodestring(g_101_gif_zlib_base64)))

  # Load the index oid from the header object.
  if ( hdr.has_key('soid') ):
    idx_oid = Oid(hdr['soid'])
  elif (hdr.has_key('oid') ):
    idx_oid = Oid(hdr['oid'])
  else:
    warn("Header is missing 'oid' or 'soid' field.  unable to generate index.html link")
