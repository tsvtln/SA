...
# TODOs:
#
# [ ] Make sure we "do the right thing" if either the -B or -A code blobs fail 
#     to compile.
#
#------------------------------------------------------------------------------
...
if ( args != '' ): args = "?%s" % args
(url_obj,headers)=urlopen.httpReply(urlopen.httpUrlRequest("https://localhost:1004/sys/${sSBOpletFileName}%s" % args,ctx=certmaster.getContext("/var/opt/opsware/crypto/spin/spin.srv"),connect_timeout=60,read_timeout=60,write_timeout=60));
info_str=""
